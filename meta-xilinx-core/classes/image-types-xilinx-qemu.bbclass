...
CONVERSION_CMD:qemu-sd-fatimg () {
	...
	rm -f ${BOOT_VOLUME_ID}.img
	mkfs.vfat -n "${BOOT_VOLUME_ID}" -S 512 -C ${BOOT_VOLUME_ID}.img $BOOT_BLOCKS
	if [ -e ${DEPLOY_DIR_IMAGE}/boot.bin ]; then
		mcopy -i ${BOOT_VOLUME_ID}.img -s  ${DEPLOY_DIR_IMAGE}/boot.bin ::/
	fi
	if [ -e ${DEPLOY_DIR_IMAGE}/boot.scr ]; then
		mcopy -i ${BOOT_VOLUME_ID}.img -s  ${DEPLOY_DIR_IMAGE}/boot.scr ::/
	fi
	if [ ${INITRAMFS_IMAGE} = ${IMAGE_BASENAME} ] && [ x"${INITRAMFS_IMAGE_BUNDLE}" != "x1" ]; then
		mcopy -i ${BOOT_VOLUME_ID}.img -s  ${IMAGE_NAME}${IMAGE_NAME_SUFFIX}.${type} ::rootfs.cpio.gz.u-boot
	fi
	dd if=${BOOT_VOLUME_ID}.img of=${QEMU_IMG} conv=notrunc seek=1 bs=$(expr ${IMAGE_ALIGNMENT} \* 1024)
}
