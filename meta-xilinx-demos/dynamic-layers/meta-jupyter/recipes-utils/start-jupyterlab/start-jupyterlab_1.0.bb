...
do_install() {
    ...
    install -m 0644 ${UNPACKDIR}/overrides.json ${D}${datadir}/jupyter/lab/settings/

    if ${@bb.utils.contains('DISTRO_FEATURES', 'sysvinit', 'true', 'false', d)}; then
       install -d ${D}${sysconfdir}/init.d/
       install -m 0755 ${UNPACKDIR}/jupyter-setup.sh ${D}${sysconfdir}/init.d/jupyter-setup.sh
    fi

    install -d ${D}${systemd_system_unitdir}
    install -m 0644 ${UNPACKDIR}/jupyter-setup.service ${D}${systemd_system_unitdir}

    install -d ${D}${systemd_user_unitdir}
    install -m 0644 ${UNPACKDIR}/jupyter-setup.service ${D}${systemd_user_unitdir}

    install -d ${D}${base_sbindir}
    install -m 0755 ${UNPACKDIR}/start-jupyter.sh ${D}${base_sbindir}/start-jupyter.sh

    install -d ${D}${sysconfdir}/jupyter/
    install -m 0644 ${UNPACKDIR}/jupyter_server_config.py ${D}${sysconfdir}/jupyter
}
